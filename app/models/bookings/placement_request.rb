...
module Bookings
  class PlacementRequest < ApplicationRecord
    ...
    scope :requiring_attention_including_attendance, -> do
      left_joins(:booking) \
        .where("bookings_bookings.id IS NULL OR (bookings_bookings.accepted_at IS NOT NULL AND bookings_bookings.date < ? AND bookings_bookings.attended IS NULL)", Date.today)
        .left_joins(:candidate_cancellation)
        .merge(Cancellation.unviewed)
        .merge(Cancellation.sent.or(Cancellation.where(id: nil)))
        .left_joins(:school_cancellation)
